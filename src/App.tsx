...
function App() {
  const { loading, session, profile, offlineAccess, signInWithEmail, signOut } = useAuth();
  ...
  useEffect(() => {
    if (loading) return;
    setScreen(session || offlineAccess ? 'app' : 'landing');
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        {screen === 'auth' && <AuthScreen loading={authLoading} error={authError} onSubmit={handleAuth} onBack={() => setScreen('landing')} />}
        {screen === 'app' && (session || offlineAccess) && (profile?.role === 'super_admin' ? <SuperAdminPanel onLogout={handleLogout} /> : <Workspace onLogout={handleLogout} />)}
        {screen !== 'auth' && !(screen === 'app' && (session || offlineAccess)) && <Landing onLogin={enterAuth} onSignup={enterAuth} />}
      </motion.div>
    </AnimatePresence>
  );
}
...
function AuthScreen({ loading, error, onSubmit, onBack }: { loading: boolean; error: string | null; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onBack: () => void }) {
  return <main className="auth-page"><div className="auth-visual"><div className="auth-visual-overlay" /><nav><button className="back-button" onClick={onBack}><X size={17} /> Close</button><Logo light /></nav><div className="auth-quote"><span className="quote-mark">“</span><h2>Clarity is a competitive advantage.</h2><p>Bring your team's best thinking into focus.</p><div className="quote-author"><span className="author-avatar">SA</span><span><strong>Sarah Ahmed</strong><small>Head of Product, Northstar</small></span></div></div><div className="auth-caption"><span>SYS/01</span><span>Human-centered operations</span></div></div><section className="auth-panel"><div className="auth-panel-inner"><div className="mobile-auth-logo"><Logo /></div><motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25, ease: [0.22, 1, 0.36, 1] }}><div className="auth-heading"><div className="status-pill"><span /> Secure workspace access</div><h1>Welcome to your workspace</h1><p>Enter your email and password to sign in. New here? An account is created automatically.</p></div><form onSubmit={onSubmit}><label>Work email<input type="email" name="email" placeholder="you@company.com" required /></label><label>Password<input type="password" name="password" placeholder="At least 8 characters" minLength={8} required /></label>{error && <div className="auth-error">{error}</div>}<button className="primary-button auth-submit" disabled={loading}>{loading ? <><LoaderCircle size={18} className="spin" /> Entering your workspace...</> : <>Enter workspace <ArrowUpRight size={17} /></>}</button></form><p className="legal">By continuing, you agree to Sysmobyte's <u>Terms</u> and <u>Privacy Policy</u>.</p></motion.div></div></section></main>;
}
